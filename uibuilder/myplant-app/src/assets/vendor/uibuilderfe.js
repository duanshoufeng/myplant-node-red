...
if (typeof require !== "undefined" && typeof io === "undefined") {
  // eslint-disable-line block-scoped-var, no-use-before-define
...
(function () {
  ...
  function makeMeAnObject(thing, property) {
    ...
    if (typeof property !== "string") {
      console.warn(
        "[uibuilderfe:makeMeAnObject] WARNING: property parameter must be a string and not: " + typeof property
      );
      property = "payload";
    }
    var out = {};
  ...
  export var uibuilder = function uibuilder() {
    ...
    self.setIOnamespace = function setIOnamespace() {
      ...
      if (ioNamespace === undefined || ioNamespace === "") {
        ...
        var u = window.location.pathname.split("/").filter(function (t) {
          return t.trim() !== "";
        });
    ...
    if ("uibuilder-webRoot" in self.cookies) {
      ...
      var fullPath = window.location.pathname.split("/").filter(function (t) {
        return t.trim() !== "";
      });
    ...
    self.set = function set(prop, val) {
      ...
      self.uiDebug(
        "debug",
        "uibuilderfe: prop set - prop: " + prop + ", val: " + (typeof val === "object" ? JSON.stringify(val) : val)
      );
    ...
    self.checkConnect = function checkConnect(delay, factor, depth = 1) {
      ...
      self.uiDebug(
        "debug",
        "uibuilderfe:checkConnect. Reconnect - Depth: " + depth + ", Delay: " + delay + ", Factor: " + factor,
        self.ioNamespace,
        self.ioPath
      );
      ...
      self.timerid = window.setTimeout(function () {
        ...
        self.uiDebug(
          "debug",
          "uibuilderfe:checkConnect timeout. SIO reconnect attempt, timeout: " + delay + ", depth: " + depth,
          self.ioNamespace,
          self.ioPath
        );
    ...
    self.ioSetup = function ioSetup() {
      ...
      self.uiDebug(
        "debug",
        "[uibuilderfe:ioSetup] About to create IO. Namespace: " +
          self.ioNamespace +
          ", Path: " +
          self.ioPath +
          ", Transport: [" +
          self.ioTransport.join(", ") +
          "]"
      );
      self.socketOptions = {
        path: self.ioPath,
      ...
      self.socket.on("connect", function ioconnect() {
        ...
        self.uiDebug(
          "info",
          "[uibuilderfe:ioSetup] SOCKET CONNECTED - # connections: " +
            self.connectedNum +
            " Namespace: " +
            self.ioNamespace,
          " Server Channel: ",
          self.ioChannels.server,
          " Control Channel: ",
      ...
      self.socket.on(self.ioChannels.server, function msgFromServer(receivedMsg) {
        self.uiDebug(
          "info",
          "[uibuilderfe:ioSetup:on-" +
            self.ioChannels.server +
            " (server)] msg received - Namespace: " +
            self.ioNamespace,
          receivedMsg
        );
      ...
      self.socket.on(self.ioChannels.control, function ctrlMsgRecvd(receivedCtrlMsg) {
        self.uiDebug(
          "debug",
          "[uibuilder:ioSetup:on-ctrl-recvd] " +
            self.ioChannels.control +
            " (control): msg received - Namespace: " +
            self.ioNamespace,
          receivedCtrlMsg
        );
        ...
        switch (receivedCtrlMsg.uibuilderCtrl) {
          ...
          case "client connect": {
            self.uiDebug(
              "info",
              `[uibuilderfe:ioSetup:${self.ioChannels.control}] Received "client connect" from server`
            );
            self.uiDebug(
              "info",
              `[uibuilderfe:ioSetup:client-connect] Server: Version=${receivedCtrlMsg.version}, Time=${receivedCtrlMsg.serverTimestamp}.`
            );
            if (self.version !== receivedCtrlMsg.version)
              console.warn(
                `[uibuilderfe:ioSetup:client-connect] Server version (${receivedCtrlMsg.version}) not the same as the client version (${self.version})`
              );
            ...
            if (self.autoSendReady === true) {
              ...
              self.uiDebug(
                "info",
                "[uibuilderfe:ioSetup:client-connect] Auto-sending ready-for-content/replay msg to server"
              );
          ...
          case "socket error": {
            console.error(
              `[uibuilderfe:ioSetup:${self.ioChannels.control}] Received "socket error" from server. ${receivedCtrlMsg.error}`
            );
            break;
          }
          ...
          default: {
            self.uiDebug(
              "info",
              `[uibuilderfe:ioSetup:${self.ioChannels.control}] Received ${receivedCtrlMsg.uibuilderCtrl} from server`
            );
      ...
      self.socket.on("disconnect", function onDisconnect(reason) {
        ...
        self.uiDebug(
          "info",
          "[uibuilderfe:ioSetup:disconnect] SOCKET DISCONNECTED - Namespace: " +
            self.ioNamespace +
            ", Reason: " +
            reason
        );
      ...
      self.socket.on("connect_error", function onConnectErr(err) {
        console.error(
          `[uibuilderfe:ioSetup:connect_error] SOCKET CONNECT ERROR - Namespace: ${self.ioNamespace}, ioPath: ${self.ioPath}, Reason: ${err.message}`,
          err
        );
    ...
    self.send = function send(msgToSend, channel, originator = "") {
      ...
      self.uiDebug(
        "log",
        "[uibuilderfe:send] Sending msg - Namespace: " + self.ioNamespace + ", Channel: " + channel,
        msgToSend
      );
    ...
    self.showToast = function showToast(msg) {
      ...
      if (
        ...
        !["", "primary", "secondary", "success", "info", "warn", "warning", "error", "danger"].includes(
          toastOptions.variant
        )
      )
        toastOptions.variant = "";
    ...
    self.showToastBVue = function showToastBVue(content, toastOptions) {
      ...
      if (!self.vueApp) {
        console.warn(
          "[uibuilder:toast] Vue app object not available, cannot create a Vue toast, falling back to vanilla."
        );
        self.showToastVanilla(content, toastOptions);
        return;
      }
      ...
      if (!self.vueApp.$bvToast) {
        console.warn(
          "[uibuilder:toast] bootstrap-vue toast component not available, cannot create a Bootstrap-Vue toast, falling back to vanilla."
        );
        self.showToastVanilla(content, toastOptions);
        return;
      }
    ...
    self.onChange("msg", function onMsgRcvd(msg) {
      ...
      if (componentRef in vueApp.$refs) {
        ...
        if (msg._uib.options) {
          self.uiDebug(
            "log",
            "🔢📈 new component instance options received for ref",
            componentRef,
            ":",
            msg._uib.options
          );
          ...
          Object.keys(msg._uib.options).forEach(function (key) {
            try {
              ...
              self.uiDebug(
                "warn",
                `[uibuilderfe:internal:onChange] Could not update prop "${key}" for component ref="${componentRef}". Error: ${e.message}`
              );
            }
          });
        }
    ...
    self.uiReturn = {
      ...
      start: function start(namespace, ioPath, vueApp) {
        ...
        self.uiDebug(
          "log",
          "[uibuilderfe:start] Calling params - namespace",
          namespace,
          "ioPath",
          ioPath,
          "vueApp",
        ...
        self.uiDebug(
          "log",
          "[uibuilderfe:start] Final Socket.IO params - namespace",
          self.ioNamespace,
          "ioPath",
          self.ioPath
        );
      ...
      eventSend: function eventSend(domevent, originator = "") {
        ...
        if (!domevent.constructor.name.endsWith("Event") || !domevent.currentTarget) {
          self.uiDebug(
            "log",
            "[uibuilderfe:eventSend] ARGUMENT NOT A DOM EVENT - use data attributes not function arguments to pass data"
          );
          return;
        }
        ...
        let id = "";
        try {
          if (target.textContent !== "") id = target.textContent.substring(0, 25);
        } catch (e) {
          /** */
        }
        try {
          if (target.name !== "") id = target.name;
        } catch (e) {
          /** */
        }
        try {
